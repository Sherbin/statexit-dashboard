...
function findCommitForTimestamp(repoPath: string, timestamp: number): string {
	...
	const output = execSync(`git log --all --before=${endOfDay} --format="%H" -1`, {
		cwd: repoPath,
		encoding: 'utf-8',
	});
...
function getCommitInfo(
	repoPath: string,
	hash: string,
): { hash: string; date: string; message: string; author: string } {
	const output = execSync(`git show --no-patch --format="%H|%cI|%s|%an" ${hash}`, {
		cwd: repoPath,
		encoding: 'utf-8',
	});
...
async function analyzeSpikeDeep(config: Config, timestamp: number, compareToPrevious: boolean = true): Promise<void> {
	console.log(`\n${'='.repeat(80)}`);
	...
	try {
		...
		const currentStats = await checkoutAndMeasure(config.repo, hash, config.old, config.ignoreOld);
		...
		if (compareToPrevious) {
			...
			// eslint-disable-next-line @typescript-eslint/naming-convention
			const _prevInfo = getCommitInfo(config.repo, prevHash);

			console.log(`📏 Measuring folder size for previous day...`);
			console.log(`   Date: ${new Date(prevTimestamp * 1000).toISOString()}`);

			const prevStats = await checkoutAndMeasure(config.repo, prevHash, config.old, config.ignoreOld);
			...
			if (Math.abs(sizeDelta) > 10000) {
				...
				try {
					const diffOutput = execSync(`git diff --stat ${prevHash} ${hash} -- ${config.old}`, {
						cwd: config.repo,
						encoding: 'utf-8',
