 ...
 */
export async function findFirstAppearance(
  repoPath: string,
  folderPath: string
): Promise<CommitInfo | null> {
  try {
    const output = execSync(
      `git log --reverse --format="%H|%ct" --diff-filter=A -- "${folderPath}"`,
      {
        cwd: repoPath,
        encoding: 'utf-8',
      }
 ...
 */
export async function findMigrationStart(
  repoPath: string,
  oldPath: string,
  newPath: string
): Promise<CommitInfo> {
  ...
  return oldAppearance.timestamp >= newAppearance.timestamp
    ? oldAppearance
    : newAppearance;
}
