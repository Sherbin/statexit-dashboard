...
function findCommitForTimestamp(repoPath: string, timestamp: number): string {
  try {
    ...
    const output = execSync(
      `git log --before=${endOfDay} --format="%H" -1`,
      {
        cwd: repoPath,
        encoding: 'utf-8',
      }
 ...
 */
function getCommitInfo(repoPath: string, hash: string): Pick<CommitAnalysis, 'hash' | 'timestamp' | 'date' | 'message' | 'author'> {
  try {
    const output = execSync(
      `git show --no-patch --format="%H|%ct|%cI|%s|%an" ${hash}`,
      {
        cwd: repoPath,
        encoding: 'utf-8',
      }
...
function analyzeFileChanges(repoPath: string, hash: string, oldPath: string): FileChange[] {
  try {
    ...
    const output = execSync(
      `git show --numstat --format="" ${hash} -- ${oldPath}`,
      {
        cwd: repoPath,
        encoding: 'utf-8',
 ...
 */
function getCommitStats(repoPath: string, hash: string): string {
  try {
    const output = execSync(
      `git show --stat ${hash}`,
      {
...
function analyzeSpike(repoPath: string, oldPath: string, timestamp: number, topN: number = 20): CommitAnalysis {
  ...
  console.log(`${'#'.padStart(4)} ${'File'.padEnd(60)} ${'Add'.padStart(8)} ${'Del'.padStart(8)} ${'Total'.padStart(10)}`);
...
function main() {
  ...
  if (results.length > 1) {
    ...
    results.forEach((result, idx) => {
      ...
      console.log(`   Changes: +${result.totalAdditions.toLocaleString()} / -${result.totalDeletions.toLocaleString()}`);
